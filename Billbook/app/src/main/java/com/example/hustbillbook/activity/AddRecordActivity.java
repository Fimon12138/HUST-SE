...
public class AddRecordActivity extends AppCompatActivity implements View.OnClickListener {
    ...
    private void commit() {
        if (Double.valueOf(moneyTv.getText().toString()) == 0) {
            ...
            return;
        } else if (moneyTv.getText().toString().charAt(moneyTv.getText().toString().length() - 1) == '.') {
            Toast.makeText(this, "请输入有效的金额！", Toast.LENGTH_SHORT).show();
            return;
        } else if (SingleCommonData.getAccountList().size() == 1) {
            Toast.makeText(this, "还没有选择账户！", Toast.LENGTH_SHORT).show();
            return;
        }
